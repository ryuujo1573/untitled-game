export function generateDefaultSkyboxEquirect(width: number, height: number): Uint8ClampedArray {
  ...
  for (let y = 0; y < h; y++) {
    ...
    const horizonBand = Math.exp(-Math.pow((v - 0.5) / 0.02, 2.0));
    r += horizonBand * 52;
    g += horizonBand * 54;
    b += horizonBand * 58;
    ...
    for (let x = 0; x < w; x++) {
      ...
      const cloudField = (cloudWaveA * 0.6 + cloudWaveB * 0.4) * 0.5 + 0.5;
      const cloudBand = Math.exp(-Math.pow((v - 0.43) / 0.12, 2.0));
      const cloudMask = clamp01((cloudField - 0.62) * 2.3) * cloudBand;

      // Subtle sun glow near one azimuth for depth without introducing seams.
      const sunTheta = Math.PI * 0.1;
      const dTheta = Math.acos(Math.max(-1, Math.min(1, Math.cos(theta - sunTheta))));
      const sunGlow = Math.exp(-Math.pow(dTheta / 0.22, 2.0)) * Math.exp(-Math.pow((v - 0.47) / 0.09, 2.0));

      const rr = Math.max(0, Math.min(255, r + cloudMask * 12 + sunGlow * 35));
      const gg = Math.max(0, Math.min(255, g + cloudMask * 12 + sunGlow * 28));
      const bb = Math.max(0, Math.min(255, b + cloudMask * 14 + sunGlow * 18));
