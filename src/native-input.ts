...
export class NativeInputManager implements InputBackend {
    ...
    update(camera: Camera, sensitivityMultiplier: number): void {
        if (!this.locked || (this.pendingDx === 0 && this.pendingDy === 0)) return;

        const sensitivity = sensitivityMultiplier;
        // Clamp outlier spikes that can occur on lock/unlock transitions.
        const maxDelta = 200;
        const dx = Math.max(-maxDelta, Math.min(maxDelta, this.pendingDx));
        const dy = Math.max(-maxDelta, Math.min(maxDelta, this.pendingDy));
