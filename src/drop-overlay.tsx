...
import { navigateToShaderpacks, setHighlightedPack } from "~/pause-menu";
      ...
      reader.readEntries(async (entries) => {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory) {
            pending.push(
              readDirectoryEntry(entry as FileSystemDirectoryEntry, rel).then(
                (sub) => {
...
function nameFromEntry(name: string): string {
  return name.replace(/\.zip$/i, "").replace(/[/\\]+$/, "") || "shaderpack";
}
    ...
    try {
      ...
      if (items && items.length > 0) {
        ...
        if (entry?.isDirectory) {
          ...
          files = await readDirectoryEntry(entry as FileSystemDirectoryEntry, "");
        }
      }
      ...
      if (!files && e.dataTransfer.files.length > 0) {
        ...
        if (droppedFile.name.toLowerCase().endsWith(".zip")) {
          ...
          const result = await extractZipToVirtualFiles(bytes);
          files = result.files;
        } else {
      ...
      getCurrentWebviewWindow()
        .onDragDropEvent(async (event) => {
          ...
          if (type === "enter" || type === "over") {
            ...
            try {
              const path = paths[0];
              const packName = nameFromEntry(path.split("/").pop() ?? "shaderpack");

              if (path.toLowerCase().endsWith(".zip")) {
                // Zip file: read bytes and extract.
                await loadShaderpack({ kind: "zip", path });
              } else {
  ...
  return (
    <Show when={dragging() || processing()}>
      <div
        ...
        classList={{ "opacity-100": dragging(), "opacity-80": !dragging() && processing() }}
      >
        <div class="flex flex-col items-center gap-4 p-12 border-2 border-dashed border-white/30 rounded-2xl">
          <div class="flex items-center justify-center w-14 h-14 rounded-full bg-primary/20">
            <Plus size={28} class="text-primary" stroke-width={2.5} />
          </div>
          <div class="text-center">
            <p class="text-white text-xl font-semibold tracking-wide">
              {processing() ? "Adding Shaderpack…" : "Add Shaderpack"}
            </p>
            <p class="text-white/50 text-sm mt-1">
              {processing() ? "Please wait" : "drop to confirm"}
            </p>
          </div>
        </div>
      </div>
...
function fileListFromMap(files: Map<string, string>): File[] {
  const out: File[] = [];
...
export function mountDropOverlay(container: HTMLElement): () => void {
