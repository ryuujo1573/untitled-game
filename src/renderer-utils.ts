...
function CreateShaderProgram(
  ...
  const linkStatus: boolean = gl.getProgramParameter(
    shaderProgram,
    gl.LINK_STATUS,
  );
  if (linkStatus) {
...
function CreateShaderMaterial(
  ...
  const vertexShader = CompileShaderSource(
    gl,
    vertexShaderSrc,
    gl.VERTEX_SHADER,
  );
  ...
  if (!vertexShader || !fragmentShader) {
    console.error(
      "Failed to compile vertex or fragment shader inside the CreateShaderMaterial function...",
    );
    return null;
  }
