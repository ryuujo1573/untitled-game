...
import {
  buildAllAtlases,
  buildAllAtlasesFromManifest,
} from "~/engine/rendering/atlas";
import { Camera } from "~/engine/rendering/camera";
import { ChatBox } from "~/ui/chat-box";
import { buildCloudMesh } from "~/engine/rendering/clouds";
import { DebugOverlay } from "~/ui/debug";
import { mountDropOverlay } from "~/ui/drop-overlay";
import { Frustum } from "~/engine/rendering/frustum";
import { captureFromRuntime, hydrateRuntime } from "~/logic/session/session-codec";
import type { GameSaveV1 } from "~/logic/session/session-types";
import { InputManager } from "~/engine/input/input";
import { PauseMenu, type QuitToTitleIntent } from "~/ui/pause-menu";
import { Physics } from "~/engine/physics/physics";
import { raycast, type RayHit } from "~/engine/physics/raycaster";
import type { IRenderer } from "~/engine/rendering/renderer-interface";
import { ResourcePackManager } from "~/environment/resource/resource-pack";
import { Settings } from "~/logic/settings/settings";
import { getShaderpackStateSnapshot } from "~/engine/shaderpack/runtime";
import { ShaderProgramRegistry } from "~/engine/shaderpack/registry";
import { STAGE_NAMES } from "~/engine/shaderpack/types";
import { generateDefaultSkyboxEquirect } from "~/engine/rendering/skybox";
import Time from "~/environment/time/time-manager";
import { CHUNK_SIZE, type Chunk } from "~/environment/world/chunk";
...
export class WebGPURenderer implements IRenderer {
  ...
  async startSession(
    ...
    const outlinePipelineLayout = device.createPipelineLayout({
      bindGroupLayouts: [outlineFrameLayout, outlineChunkLayout],
    });
    const deferredPipelineLayout = device.createPipelineLayout({
      bindGroupLayouts: [deferredLayout],
    });
    const cloudsPipelineLayout = device.createPipelineLayout({
    ...
    const chatBox = new ChatBox(() => input.requestLock());
    chatBox.mount(document.getElementById("chat-root")!);
    ...
    world.chunks.forEach(uploadChunk);
    ...
    function frame(): void {
      ...
      const cloudsMode =
        shaderpackSnapshot.manifest?.properties.clouds ?? "fancy";
      ...
      const ambR =
        lerp(lerp(0.03, 0.75, twilight), 1.0, dayFactor) * hdrScale * brt;
      const ambG =
        lerp(lerp(0.03, 0.48, twilight), 0.92, dayFactor) * hdrScale * brt;
      const ambB =
        lerp(lerp(0.1, 0.22, twilight), 0.8, dayFactor) * hdrScale * brt;
