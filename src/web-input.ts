...
export class WebInputManager implements InputBackend {
    ...
    update(camera: Camera, sensitivityMultiplier: number): void {
        ...
        if (this.dx !== 0 || this.dy !== 0) {
            ...
            const maxDelta = 120;
            const finalDx = Math.max(-maxDelta, Math.min(maxDelta, this.dx));
            const finalDy = Math.max(-maxDelta, Math.min(maxDelta, this.dy));

            camera.rotate(finalDx * webSensitivity, finalDy * webSensitivity);
            this.dx = 0;
            this.dy = 0;
        }
    }
