...
describe("parseShadersProperties", () => {
  test("parses alpha/blend/flip/options/custom uniforms and cloud mode", () => {
    ...
    expect(parsed.alphaTests.get("gbuffers_terrain")).toEqual({
      func: "GREATER",
      ref: 0.1,
    });
    ...
    expect(parsed.flips.get("composite.colortex4")).toBe(true);
    expect(parsed.options.get("SHADOW_QUALITY")).toBe("1");
    expect(parsed.options.get("AMBIENT_OCCLUSION")).toBe("ON");
    expect(parsed.screen).toEqual(["SHADOW_QUALITY", "AMBIENT_OCCLUSION"]);
    expect(parsed.variables.get("myVar")).toEqual({ type: "float", expr: "sin(frameTimeCounter)" });
    expect(parsed.uniforms.get("customTime")).toEqual({ type: "float", expr: "frameTimeCounter * 0.5" });
    expect(parsed.uniforms.get("customColor")).toEqual({ type: "vec3", expr: "vec3(myVar, 0.0, 1.0)" });
    expect(parsed.clouds).toBe("fancy");
    expect(parsed.warnings.some((w) => w.includes("Unsupported shaders.properties key: clouds"))).toBe(false);
  });
});
