...
describe("preprocessShader", () => {
  test("keeps #version first, injects macros, resolves includes", () => {
    ...
    const out = preprocessShader({
      ...
      includeMap: new Map([
        ["lib/common.glsl", "#define UTIL vec3(0.0)"],
      ]),
      platform: "mac",
    });
    ...
    expect(
      out.code.includes("#define ROUGHLY_A_3D_GAME 1"),
    ).toBe(true);
    expect(out.code.includes("#define MC_OS_MAC")).toBe(
      true,
    );
    expect(
      out.code.includes(
    ...
    expect(
      out.code.includes("#define SHADOW_QUALITY 1"),
    ).toBe(true);
    expect(
      out.code.includes("#define UTIL vec3(0.0)"),
    ).toBe(true);
  });
  ...
  test("detects include cycles", () => {
    ...
    expect(
      out.warnings.some((w) =>
        w.includes("Include cycle detected"),
      ),
    ).toBe(true);
  });
});
